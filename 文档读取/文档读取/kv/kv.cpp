...
#include <fstream>
#include <iostream>
//#include "opencv2/core/core.hpp" If there is an error, replace the next line with this line
#include<assert.h>
...
void Kv::add(const std::string &key, const std::string &value) {
	if (data_.find(key) != data_.end()) {	//safed check, judge whether the key already exists
		fprintf(stderr,
