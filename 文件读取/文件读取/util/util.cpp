...
// The return value is the full path to each file
std::vector<std::string> Utils::getFiles(const std::string &folder,
	...
	while (!subfolders.empty()) {
		...
		subfolders.pop_back();	//delete the last element

		struct _finddata_t file_info;
		//Get the specified path filespec the first file (or folder),filespec supports wildcards(*,?)
		...
		while (file_handler != -1) {
			...
			if (file_info.attrib & _A_SUBDIR) {	//whether to include Subdirectories
				// it's a sub folder
				if (all) {
					// will search sub folder
					std::string folder(current_folder);
					folder.pop_back(); //pop_back the '*'
				...
				file_path.assign(current_folder.c_str()).pop_back();	//get current directory,and delete the last '*'
...
#endif
	return last_slash;	//the last '\\' or '/' position
}
